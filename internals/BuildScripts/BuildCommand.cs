...
static class BuildCommand {
  ...
  static string[] GetEnabledScenes () {
    return (
      from scene in EditorBuildSettings.scenes where scene.enabled where!string.IsNullOrEmpty (scene.path) select scene.path
    ).ToArray ();
  }
  ...
  static bool TryConvertToEnum<TEnum> (
    this string strEnumValue, out TEnum value
  ) {
